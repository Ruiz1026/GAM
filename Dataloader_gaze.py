...
class TestingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if image.shape[:2] != ori_np_mask.shape[:2]:
            ori_np_mask = cv2.resize(ori_np_mask, (image.shape[1], image.shape[0]))
        ...
        if self.prompt_path is None:
            boxes = get_boxes_from_mask(mask, max_pixel = 5)
...
class TrainingDataset(Dataset):
    def __init__(self, data_dir, image_size=256, mode='train', requires_name=True, point_num=3, mask_num=1):
        """
        ...
        #dataset = json.load(open(os.path.join(data_dir, f'json_train/image2label_{mode}3.json'), "r"))
        #heatmap = json.load(open(os.path.join(data_dir, f'json_heatmap/image2heatmap_{mode}3.json'), "r"))
        #gaze_point = json.load(open(os.path.join(data_dir, f'json_points/image2points_{mode}3.json'), "r"))
        dataset = json.load(open(os.path.join(data_dir, f'image2label_{mode}.json'), "r"))
        heatmap = json.load(open(os.path.join(data_dir, f'image2heatmap_{mode}.json'), "r"))
        gaze_point = json.load(open(os.path.join(data_dir, f'image2points_{mode}.json'), "r"))
        self.image_paths = list(dataset.keys())
        self.label_paths = list(dataset.values())
        print(len(self.label_paths))
        self.heatmap_paths = list(heatmap.values())
        self.gaze_point_paths = list(gaze_point.values())
    def __getitem__(self, index):
        """
        ...
        pre_heatmap = cv2.imread(self.heatmap_paths[index])
        points_path=self.gaze_point_paths[index]
        point_coords = np.load(points_path)['points']
        point_label=[1] * point_coords.shape[0]
        #point_label=[1] * 50
        #point_label=[1] * 80
        #point_label=[1] * 200
        augments = transforms(image=image, mask=pre_mask,heatmap=pre_heatmap)
        heatmap_aug = transforms(image=pre_heatmap)
        image_tensor, mask_tensor,heatmap_tensor = augments['image'], augments['mask'].to(torch.int64),heatmap_aug['image']
        boxes = get_boxes_from_mask(mask_tensor)
        #point_coords, point_label = init_point_sampling(mask_tensor, self.point_num)

        masks_list.append(mask_tensor)
        boxes_list.append(boxes)
        point_coords_list.append(torch.as_tensor(point_coords))
        point_labels_list.append(torch.as_tensor(point_label))
        ...
        image_input["heatmap"] = heatmap_tensor.unsqueeze(0)
        image_input["boxes"] = boxes
        image_input["point_coords"] = point_coords
        image_input["point_labels"] = point_labels
