...
def init_point_sampling(mask, get_point=1):
    ...
    if get_point == 1:
        ...
        if fg_size == 0:
            ...
            coords = bg_coords[bg_indices]
            labels = np.zeros(get_point).astype(int)
...
def generate_point1(masks, labels, low_res_masks, batched_input, point_num):
    masks_clone = masks.clone()
    masks_sigmoid = torch.sigmoid(masks_clone)
    masks_binary = (masks_sigmoid > 0.5).float()
