...
class TrainingDataset(Dataset):
    def __init__(self, data_dir, image_size=256, mode='train', requires_name=True, point_num=3, mask_num=1):
        ...
        # dataset = json.load(open(os.path.join(data_dir, f'json_train/image2label_{mode}6.json'), "r"))
        # heatmap = json.load(open(os.path.join(data_dir, f'json_heatmap/image2heatmap_{mode}6.json'), "r"))
        # gaze_point = json.load(open(os.path.join(data_dir, f'json_points/image2points_{mode}6.json'), "r"))
    ...
    def __getitem__(self, index):
        ...
        #point_label=[1] * 80
        #point_label=[1] * 200
        augments = transforms(image=image, mask=pre_mask,heatmap=pre_heatmap,is_check_shapes=False)
